...
import { LineDiscrepancy as LineDiscrepancyType, Game } from '@/lib/types'
import { formatOdds, impliedProbability } from '@/lib/odds-utils'
...
export default function LineDiscrepancy({ discrepancies, games }: Props) {
  ...
  const getGameDetails = (gameId: string, betType: string) => {
    const game = games.find(g => g.id === gameId)
    if (!game) return null
    
    return game.bookmakers.map(book => {
      const market = book.markets.find(m => 
        m.key === 'h2h' || m.key === 'spreads' || m.key === 'totals'
      )
      const outcome = market?.outcomes.find(o => 
        o.name === betType || o.name.includes('Over') || o.name === game.homeTeam
      )
      return {
        book: book.title,
            ...
            <tbody className="divide-y divide-slate-800">
              <AnimatePresence>
                {filteredDiscrepancies.map((disc, index) => (
                  <motion.tr
                    key={`${disc.gameId}-${disc.market}-${disc.betType}`}
                    ...
                    onClick={() => setExpandedRow(
                      expandedRow === `${disc.gameId}-${disc.market}` 
                        ? null 
                        : `${disc.gameId}-${disc.market}`
                    )}
                  >
