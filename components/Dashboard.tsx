...
import LineDiscrepancy from './LineDiscrepancy'
import EVCalculator from './EVCalculator'
import PlayerPropAnalyzer from './PlayerPropAnalyzer'
import { 
  MOCK_GAMES, 
  generateLineDiscrepancies, 
  generateEVBets, 
  MOCK_PLAYER_PROPS 
} from '@/lib/mock-data'
import { LineDiscrepancy as LineDiscType, EVBet, Game, PlayerProp } from '@/lib/types'
...
export default function Dashboard() {
  ...
  const [discrepancies, setDiscrepancies] = useState<LineDiscType[]>([])
  ...
  const loadData = useCallback(() => {
    ...
    setTimeout(() => {
      setGames(MOCK_GAMES)
      setDiscrepancies(generateLineDiscrepancies())
      setEvBets(generateEVBets())
      setPlayerProps(MOCK_PLAYER_PROPS)
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-xl tracking-tight">
                  Sharp<span className="text-green-400">Edge</span>
                </h1>
                <p className="text-xs text-slate-500">Sports Betting Analytics</p>
              </div>
            </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'overview' && (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="card p-4 card-hover">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-400">Games Today</span>
                    <BarChart3 className="w-4 h-4 text-purple-400" />
                  </div>
                  <p className="text-3xl font-bold mt-2">{games.length}</p>
                  <p className="text-xs text-slate-500 mt-1">NBA matchups</p>
                </div>
              </div>
              ...
              <div className="card p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {games.map(game => (
                    <div key={game.id} className="p-4 bg-slate-800/30 rounded-lg">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-400">@ {game.homeTeam}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'discrepancies' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <LineDiscrepancy discrepancies={discrepancies} games={games} />
            </motion.div>
          )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-sm text-slate-500">
            <div className="flex items-center gap-2">
              ...
              <span>SharpEdge Analytics</span>
            </div>
            <div className="flex items-center gap-4">
              <span>Data via The Odds API</span>
              <span>•</span>
