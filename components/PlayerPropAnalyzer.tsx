...
export default function PlayerPropAnalyzer({ playerProps }: Props) {
  ...
  useEffect(() => {
    if (currentProp) {
      const statLines: Record<string, Record<StatType, number>> = {
        jokic: { PTS: 29.5, REB: 11.5, AST: 9.5, BLK: 0.5, STL: 1.5, PRA: 50.5, '3PM': 1.5 },
        lebron: { PTS: 25.5, REB: 8.5, AST: 8.5, BLK: 0.5, STL: 1.5, PRA: 42.5, '3PM': 2.5 },
        sga: { PTS: 31.5, REB: 5.5, AST: 6.5, BLK: 0.5, STL: 2.5, PRA: 43.5, '3PM': 2.5 },
        luka: { PTS: 33.5, REB: 9.5, AST: 9.5, BLK: 0.5, STL: 1.5, PRA: 52.5, '3PM': 3.5 },
        giannis: { PTS: 30.5, REB: 11.5, AST: 6.5, BLK: 1.5, STL: 1.5, PRA: 48.5, '3PM': 0.5 },
        curry: { PTS: 27.5, REB: 4.5, AST: 5.5, BLK: 0.5, STL: 1.5, PRA: 37.5, '3PM': 5.5 }
      }
      setPropLine(statLines[selectedPlayer]?.[selectedStat] || 20)
    }
  }, [selectedPlayer, selectedStat, currentProp])
        ...
        <div className="lg:col-span-3 card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4 text-xs">
              ...
              <div className="flex items-center gap-1">
                <div className="w-2 h-0 border-t-2 border-dashed border-yellow-500 w-6" />
          ...
          <div className="h-[300px] relative">
            ...
            <div 
              className="absolute left-[60px] right-[10px] border-t-2 border-dashed border-yellow-500 pointer-events-none"
              style={{ 
                bottom: `${(propLine / Math.max(...filteredGames.map(g => getStatValue(g)), propLine + 5)) * 100}%` 
              }}
            />
          </div>
        </div>
